    ...
    <header className="w-full sticky top-0 z-50 bg-white shadow-md border-b border-gray-100">
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
        ...
        <div className={`flex justify-center mb-4 transition-all duration-700 ${mounted ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}>
          <Link to="/home">
            <img
              ...
              style={{ transform: flip ? "rotateY(180deg)" : "rotateY(0deg)", transition: "transform 0.6s ease-in-out" }}
            />
          </Link>
        </div>
        ...
        <div className={`flex flex-col lg:flex-row items-center justify-between gap-4 sm:gap-6 transition-all duration-700 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4"}`}>
          ...
          {/* Search */}
          <form onSubmit={handleSearch} className="flex items-center w-full max-w-md bg-gray-50 border border-gray-200 rounded-full px-4 py-2 focus-within:ring-2 focus-within:ring-pink-400 transition-all">
            <input type="text" placeholder="Search sarees, categories..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="flex-1 bg-transparent outline-none text-sm text-gray-700 placeholder-gray-400" />
            <button type="submit" className="ml-3 bg-linear-to-r from-pink-500 to-red-500 text-white px-4 py-1.5 rounded-full text-sm font-semibold hover:from-pink-600 hover:to-red-600 transition-all">
              Search
            </button>
          </form>
          ...
          <div className="flex gap-3 sm:gap-4 items-center">
            {!user ? (
              <Link to="/login">
                <Button text="Login" property="bg-gradient-to-r from-pink-500 to-red-500 hover:from-pink-600 hover:to-red-600 text-white px-6 py-2.5 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 font-semibold cursor-pointer" />
              </Link>
            ) : (
              <>
                {/* Wishlist */}
                ...
                <div className="relative group">
                  <div className="flex items-center gap-2 cursor-pointer px-4 py-2 rounded-full hover:bg-gray-50 transition-all duration-300">
                    ...
                    <span className="hidden sm:block font-semibold text-gray-700 max-w-25 truncate">{user.name}</span>
                  </div>
                  <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform origin-top scale-95 group-hover:scale-100 overflow-hidden">
                    ...
                    <Link to="/orders" className="flex items-center gap-3 px-4 py-2.5 hover:bg-pink-50 transition-colors duration-200 text-gray-700 hover:text-pink-500">
                      <span className="text-sm font-medium">My Orders</span>
                    </Link>
                    <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-red-50 transition-colors duration-200 text-gray-700 hover:text-red-500 border-t border-gray-100">
